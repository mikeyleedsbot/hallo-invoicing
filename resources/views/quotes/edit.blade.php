        ...
        <form action="{{ route('quotes.update', $quote) }}" method="POST" @submit="submitForm" class="space-y-6">
            ...
            <div class="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-gray-800 dark:to-gray-700 rounded-lg shadow-sm border border-blue-200 dark:border-gray-600 p-6">
                <div class="grid grid-cols-3 gap-6">
                    ...
                    <div class="text-center">
                        <div class="text-sm text-gray-600 dark:text-gray-400 mb-1">Totaal (incl. BTW)</div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <select name="customer_id" required placeholder="Zoek klant..." x-tom-select="{maxOptions: null}"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                            <option value=""></option>
                            @foreach($customers as $customer)
                                <option value="{{ $customer->id }}" {{ $quote->customer_id == $customer->id ? 'selected' : '' }} data-postal="{{ $customer->postal_code }}">{{ $customer->name }}@if($customer->company_name) ({{ $customer->company_name }})@endif @if($customer->address || $customer->city) | @if($customer->address){{ $customer->address }}, @endif{{ $customer->city }}@endif</option>
                            @endforeach
                        </select>
                        @error('customer_id')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select name="template_id" x-tom-select="{placeholder: 'Selecteer template...'}"
                            ...
                            @foreach($templates as $template)
                                <option value="{{ $template->id }}" {{ $quote->template_id == $template->id || (!$quote->template_id && $template->is_default_quote) ? 'selected' : '' }}>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 mb-4 p-3 rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700">
                    ...
                    <div class="inline-flex rounded-lg border border-gray-300 dark:border-gray-500 overflow-hidden">
                        <button type="button" @click="pricesIncludeVat = false"
                        ...
                        <button type="button" @click="pricesIncludeVat = true"
                            class="px-4 py-2 text-sm font-medium border-l border-gray-300 dark:border-gray-500 transition-colors"
                            :class="pricesIncludeVat
                                ? 'bg-blue-600 text-white'
                                : 'bg-white text-gray-700 hover:bg-gray-100 dark:bg-gray-600 dark:text-gray-200 dark:hover:bg-gray-500'">
                            Incl. BTW
                        ...
                        <thead>
                            <tr class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase">
                                ...
                                <th class="text-left pb-2 w-32"
                                    x-text="pricesIncludeVat ? 'Prijs/stuk incl.' : 'Prijs/stuk excl.'">Prijs/stuk excl.</th>
                                        ...
                                        <div class="border border-gray-200 dark:border-gray-600 rounded-lg p-3 bg-gray-50 dark:bg-gray-700">
                                            ...
                                            <div class="mt-2 pt-2 border-t border-gray-300 dark:border-gray-600 text-right">
                                                <span class="text-xs text-gray-600 dark:text-gray-400"
                                                    x-text="(pricesIncludeVat ? 'Regeltotaal incl. BTW' : 'Regeltotaal excl. BTW') + ': '">Regeltotaal: </span>
    ...
    <script>
        ...
        function quoteForm() {
            return {
                ...
                productSearch: '',
                pricesIncludeVat: {{ $quote->prices_include_vat ? 'true' : 'false' }},
                ...
                lineAmounts(line) {
                    const rate = Math.max(0, parseFloat(line.vat_rate) || 0);
                ...
                addProduct(product) {
                    ...
                    const unitPrice = this.pricesIncludeVat
