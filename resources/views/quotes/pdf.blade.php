            ...
            <div class="info-block">
                ...
                <div class="info-content">
                    ...
                    @if($quote->customer->address)
                        <div style="margin-top: 5px;">{{ $quote->customer->address }}</div>
                    @endif
                    @if($quote->customer->postal_code || $quote->customer->city)
                        <div>{{ $quote->customer->postal_code }} {{ $quote->customer->city }}</div>
                    @endif
                    @if($quote->customer->country)
                        <div>{{ $quote->customer->country }}</div>
                    @endif
                    @if($quote->customer->vat_number)
                        <div style="margin-top: 5px;">BTW: {{ $quote->customer->vat_number }}</div>
                    @endif
                </div>
            </div>

            <div class="info-block">
                <div class="info-label">Offertedetails</div>
                <div class="info-content">
                    <div class="date-grid">
                        <div class="date-item">
                            <span class="date-label">Offertedatum:</span>
                            <span class="date-value">{{ $quote->quote_date->format('d-m-Y') }}</span>
                        </div>
                        <div class="date-item">
                            <span class="date-label">Geldig tot:</span>
                            <span class="date-value">{{ $quote->valid_until->format('d-m-Y') }}</span>
                        </div>
                        <div class="date-item">
                            <span class="date-label">Geldigheid:</span>
                            <span class="date-value">{{ $quote->valid_days }} dagen</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <table class="invoice-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 13%; text-align: right;">Prijs{{ $quote->prices_include_vat ? ' incl.' : '' }}</th>
                    <th style="width: 10%; text-align: center;">BTW</th>
                    <th style="width: 15%; text-align: right;">Totaal{{ $quote->prices_include_vat ? ' incl.' : '' }}</th>
                </tr>
            </thead>
            <tbody>
