    ...
    <div class="max-w-4xl mx-auto p-8">
        ...
        <div class="border-b-4 border-blue-600 pb-6 mb-8">
            <div class="flex justify-between items-start">
                ...
                <div class="text-right">
                    <h1 class="text-4xl font-bold text-blue-900 mb-2">FACTUUR</h1>
                    <p class="text-xl font-semibold text-gray-600 mb-3">{{ $invoice->invoice_number }}</p>
            ...
            <div>
                ...
                <div class="bg-gray-50 border-l-4 border-blue-600 p-4">
                    ...
                    @if($invoice->customer->country)
                        <p class="text-gray-600">{{ $invoice->customer->country }}</p>
                    @endif
                    @if($invoice->customer->vat_number)
                        <p class="text-gray-600 mt-2">BTW: {{ $invoice->customer->vat_number }}</p>
                    @endif
                </div>
            </div>
            
            <div>
                <h3 class="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Factuurdetails</h3>
                <div class="bg-gray-50 border-l-4 border-blue-600 p-4 space-y-2">
                    <div class="flex justify-between">
                        <span class="font-semibold text-gray-700">Factuurdatum:</span>
                        <span class="text-gray-900">{{ $invoice->invoice_date->format('d-m-Y') }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="font-semibold text-gray-700">Vervaldatum:</span>
                        <span class="text-gray-900">{{ $invoice->due_date->format('d-m-Y') }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="font-semibold text-gray-700">Betalingstermijn:</span>
                        <span class="text-gray-900">{{ $invoice->payment_terms }} dagen</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end mb-8">
            <div class="w-96">
                ...
                <div class="border-b border-gray-200 py-3 flex justify-between">
                    ...
                    <span class="font-bold text-gray-900">€ {{ number_format($invoice->vat_amount, 2, ',', '.') }}</span>
                </div>
                <div class="bg-blue-900 text-white py-4 px-4 flex justify-between text-lg">
                    <span class="font-bold">TOTAAL (incl. BTW)</span>
                    <span class="font-bold">€ {{ number_format($invoice->total, 2, ',', '.') }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
            ...
            <div class="space-y-2 text-sm">
                ...
                <div class="flex">
                    <span class="font-semibold text-gray-700 w-40">Betalen voor:</span>
                    <span class="text-gray-900">{{ $invoice->due_date->format('d-m-Y') }}</span>
                </div>
            </div>
        </div>
