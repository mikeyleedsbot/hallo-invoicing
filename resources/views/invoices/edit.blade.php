                    ...
                    <div>
                        ...
                        <select name="customer_id" required placeholder="Zoek klant..." x-tom-select="{maxOptions: null}"
                            ...
                            @foreach($customers as $customer)
                                <option value="{{ $customer->id }}" {{ $invoice->customer_id == $customer->id ? 'selected' : '' }} data-postal="{{ $customer->postal_code }}">{{ $customer->name }}@if($customer->company_name) ({{ $customer->company_name }})@endif @if($customer->address || $customer->city) | @if($customer->address){{ $customer->address }}, @endif{{ $customer->city }}@endif</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="template_id" x-tom-select="{placeholder: 'Selecteer template...'}"
                            ...
                            @foreach($templates as $template)
                                <option value="{{ $template->id }}" {{ $invoice->template_id == $template->id || (!$invoice->template_id && $template->is_default_invoice) ? 'selected' : '' }}>
                                    {{ $template->name }}@if($template->is_default_invoice) (standaard)@endif
                                </option>
                            @endforeach
                        </select>
                    </div>
